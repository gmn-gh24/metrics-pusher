...
### 2.1 Example — full datagram (captured from real hardware on v5.12.0, an idle desktop; every field is as the sender produced it — nothing is spliced)

```json
{"v":1,"gpu":"NVIDIA GeForce RTX 3090 Ti","host":"G-MONSTER","temp":41,"load":1,"vramUsed":3266,"vramTotal":24564,"fan":0,"power":5,"watts":23,"limitW":477,"clock":210,"vramClock":405,"cpu":"AMD Ryzen 9 9950X","cpuLoad":2,"ramUsed":24152,"ramTotal":48696,"diskFree":187,"diskTotal":837,"win":"11 23H2","av":0,"reboot":0,"fw":1,"up":99327}
```
(338 bytes)
...
before its timer starts. The harness therefore does that priming as an explicit first
pass and publishes the second. A real sender's **first** datagram normally already
carries `cpuLoad`, and its `av`/`reboot`/`fw` are best-effort against a background
refresh — see §4 and §5 for the actual first-datagram semantics.

`power`, `watts` and `limitW` are three views of **one** milliwatt pair: 23 W against
this board's 477 W enforced limit is the 5 % on the same line (a second capture on the
same machine read `"power":53,"watts":254` — the same ratio). Since v5.12.0 the
...
board-specific: do not hardcode a denominator. This capture is at idle — hence
...
| **Document version** — currently `1.16` | The footer of this file | On any edit to this document, including corrections that changed no behaviour |
