...
### 2.1 Example — full datagram (captured from real hardware on MetricsPusher next, an idle desktop; every field is as the sender produced it — nothing is spliced)

```json
{"v":1,"gpu":"NVIDIA GeForce RTX 3090 Ti","host":"G-MONSTER","temp":43,"load":1,"vramUsed":2208,"vramTotal":24564,"fan":0,"power":6,"watts":28,"limitW":477,"clock":210,"vramClock":405,"cpu":"AMD Ryzen 9 9950X","cpuLoad":3,"cpuTemp":64.875,"cpuWatts":43,"nvmeTemp":48,"ramUsed":18905,"ramTotal":48696,"diskFree":181,"diskTotal":837,"win":"11 23H2","av":0,"reboot":0,"fw":1,"up":190346}
```
(384 bytes)
...
can show all four new fields at once.

The three new sensor values also show the formatting §4.1 warns about: `cpuTemp` is
`64.875` because AMD Tdie decodes in 0.125 °C steps, while `nvmeTemp` is a whole `48`
...
where the real sender primes the PDH baseline, the RAPL energy accumulator and the first
OS-health refresh before its timer starts. The harness therefore does that priming as an
explicit first pass and publishes the second. A real sender's **first** datagram normally
already carries `cpuLoad` but **not** `cpuWatts` — the first energy sample only
establishes a baseline, so watts appear from the second tick — and its `av`/`reboot`/`fw`
are best-effort against a background refresh. See §4 and §5 for the actual
first-datagram semantics.

`power`, `watts` and `limitW` are three views of **one** milliwatt pair: 28 W against
this board's 477 W enforced limit is the 6 % on the same line
(`round(28 × 100 ÷ 477) = 6`). Since v5.12.0 the
...
and must be read as an absolute figure: 43 W is package draw with no limit beside it on
...
| **Sender version** — currently `1.0.0` | `MetricsPusher.csproj` and the app's release tag. **Never transmitted** | On any app release, wire-affecting or not |
| **Document version** — currently `1.18` | The footer of this file | On any edit to this document, including corrections that changed no behaviour |
...
- String caps (63 / 16) are measured in **JSON-encoded bytes**, not characters
  ...
  v5.8.0+; see §3.2 for older senders). Since the wire form is ASCII (escaped), an
  ...
  (`gpu`/`host`/`cpu`) and 17-byte (`win`) destinations — but per §3.2, bound the copy
  regardless.
...
- Intra-datagram skew: **fields in one datagram are not all of the same age.** The
  ...
  | `host`, `cpu`, `win`, `diskTotal` | session | the sender's whole uptime (static facts) |
...
- ArduinoJson unescapes `\uXXXX` sequences automatically; decoded strings from v5.8.0+
  ...
  with `strlcpy` anyway, because pre-v5.7.1 senders truncate nothing (§3.2).
