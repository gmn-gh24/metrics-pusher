...
| **Document version** — currently `1.20` | The footer of this file | On any edit to this document, including corrections that changed no behaviour |
...
- **Permanent / structural:** the sender's current backend or the board simply cannot
  ...
  sender, `fan` on boards whose driver does not expose a fan, and **every `gpu*` key at once
  on a sender with no NVIDIA GPU** (§7.1). Not a failure and not worth rendering as one; see
...
### 7.1 GPU availability is not a gate

**A sender does not need an NVIDIA GPU.** The push loop starts as soon as a display answers
discovery; the GPU probe runs beside it and gates nothing. On a machine where neither stack
can read an NVIDIA GPU, every `gpu*` key is absent for the whole session while the CPU,
NVMe, network, RAM, disk and OS fields report normally — an absence like any other in §5,
meaning "unknown", never zero. **Since v5.10.0 the probe asks both stacks:** NVAPI first,
and NVML only if NVAPI declines. A board only NVML can read (a TCC-mode compute card, some
vGPU adapters) therefore reports where a pre-v5.10.0 sender was silent; nothing changes for
a machine where NVAPI works, which is asked first and answers first.

Two timing consequences follow, both already covered by §5:

- **At startup the probe can lose the race with the first datagram.** The loop does not wait
  for it, so on a machine that *does* have a GPU the first datagram — rarely a few after it —
  can carry no `gpu*` keys. They appear on the first tick after the probe succeeds, including
  when it outlives its 30 s startup wait. Render this as "unknown", not as "no GPU": the
  reference consumer's "absent key ⇒ keep last-good" rule (§8.1) already handles it.
- **Mid-session GPU loss no longer silences the sender.** When GPU monitoring becomes
  unavailable the `gpu*` keys drop out together (§5) and every other field keeps arriving.
  Only a datagram in which *every* live metric is missing at once — GPU, CPU, NVMe, network,
  RAM and disk — is suppressed (§7).

What does gate the sender entirely is the **display**: it derives the display address from
its own IPv4 and probes it, and if nothing ever answers it sends nothing for the whole
session (§1.1). Consumers must not assume every PC on the subnet reports.

> **Senders before MetricsPusher v1.0.2** did gate on the GPU: the push loop started only
> once NVIDIA monitoring reported available, and a machine neither stack could read never
> sent anything. A consumer written against those senders needs no change — it is receiving
> a superset of what it saw before — but it can no longer infer "this PC has an NVIDIA GPU"
> from the mere fact that a datagram arrived. Test for the `gpu` key instead.
